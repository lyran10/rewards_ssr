...
import { useGetAdminEmployeeDetails } from '../../../customHooks/useGetAdminEmployeeDetails'

export const AdminEmployeeDetails = () => {
  const { dispatch } = useRedux()
  const {handleChange, data, setData} = UseInput()
  const {loading, employeeDetails, fetchOffers} = useGetAdminEmployeeDetails()
...
//   },
  ...
  const displayHeader = () => {
      let headers = ["Sr.no", "Employee Name", "Contact", "Address", "Email ID" ,"Reg Date", "ULB", "District", "Division", "State", "Balance", "Tolal Claim", "Total Redeem" ]
  ...
  return (
      ...
      filtered.map((item, index : number) => {
          return (
              <tr key={index} className={`${index % 2 !== 0 ? bgColors.white_smoke : ""} border-b h-[50px] text-center body relative text-[#475569]`}>
                  ...
                  <td className="p-2 font-[600] text-start">{item.employeeName}</td>
                  <td className="p-2 font-[600] text-start">{item.contactNo}</td>
                  <td className="p-2 font-[600] text-start">{item.address}</td>
                  <td className="p-2 font-[600] text-start">{item.emailId}</td>
    ...
    return (
      <section className={`relative rounded-md w-full h-full shadow-chart px-5 py-3 flex flex-col justify-start items-center gap-3 ${bgColors.white}`}>
       <header className='font-semibold text-[15px] self-start'>Employee Details</header>
